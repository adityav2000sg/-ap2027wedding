...
export async function fetchSnapshot(
...
): Promise<WeddingSnapshot> {
  {
    const [
      ...
      db.weddingMember.findMany({ where: { weddingId }, include: { user: true }, orderBy: { createdAt: "asc" } }),
      db.venue.findMany({ where: { weddingId, ...active }, orderBy: { createdAt: "asc" } }),
      db.event.findMany({ where: { weddingId, ...active }, orderBy: [{ date: "asc" }, { startMinute: "asc" }] }),
      db.household.findMany({ where: { weddingId, ...active }, orderBy: { name: "asc" } }),
      db.guest.findMany({ where: { weddingId, ...active }, orderBy: { createdAt: "asc" } }),
      db.eventInvitation.findMany({ where: { event: { weddingId } }, orderBy: { createdAt: "asc" } }),
      db.vendor.findMany({
        where: { weddingId, ...active },
        include: { events: true, attributes: true }, orderBy: { createdAt: "asc" } }),
      db.contract.findMany({ where: { weddingId }, orderBy: { createdAt: "asc" } }),
      db.budgetCategory.findMany({ where: { weddingId, ...active }, orderBy: { sortOrder: "asc" } }),
      db.budgetItem.findMany({ where: { weddingId, ...active }, orderBy: { createdAt: "asc" } }),
      db.payment.findMany({ where: { weddingId, ...active }, orderBy: { dueDate: "asc" } }),
      db.payer.findMany({ where: { weddingId }, orderBy: { sortOrder: "asc" } }),
      db.currencyRate.findMany({ where: { weddingId }, orderBy: { effectiveDate: "asc" } }),
      db.task.findMany({
        where: { weddingId, ...active },
        include: {
      ...
      db.taskDependency.findMany({ where: { task: { weddingId } }, orderBy: { id: "asc" } }),
      db.timelineEntry.findMany({ where: { weddingId, ...active }, orderBy: { createdAt: "asc" } }),
      db.timelineDependency.findMany({ where: { entry: { weddingId } }, orderBy: { id: "asc" } }),
      db.hotel.findMany({ where: { weddingId, ...active }, include: { rooms: true }, orderBy: { createdAt: "asc" } }),
      db.stay.findMany({ where: { weddingId }, orderBy: { createdAt: "asc" } }),
      db.travelRecord.findMany({ where: { weddingId }, orderBy: { createdAt: "asc" } }),
      db.transportVehicle.findMany({ where: { weddingId, ...active }, orderBy: { id: "asc" } }),
      db.transportJourney.findMany({ where: { weddingId }, include: { passengers: true }, orderBy: { createdAt: "asc" } }),
      db.wardrobePerson.findMany({ where: { weddingId }, orderBy: { sortOrder: "asc" } }),
      db.outfit.findMany({
        where: { weddingId, ...active },
        include: { accessories: true, fittings: true }, orderBy: { createdAt: "asc" } }),
      db.jewelleryItem.findMany({ where: { weddingId, ...active }, orderBy: { createdAt: "asc" } }),
      db.document.findMany({
        where: { weddingId, ...active },
        include: { links: true, uploadedBy: true }, orderBy: { createdAt: "asc" } }),
      db.responsibility.findMany({ where: { weddingId }, orderBy: { createdAt: "asc" } }),
      db.milestone.findMany({ where: { weddingId }, orderBy: { id: "asc" } }),
      db.alertDismissal.findMany({ where: { weddingId }, orderBy: { id: "asc" } }),
    ]);
    ...
    return {
      wedding: {
        ...
        invitationStage: wedding.invitationStage,
      ...
      households: households.map((h) => ({
        ...
        saveTheDateSentAt: h.saveTheDateSentAt,
        rsvpSentAt: h.rsvpSentAt,
        rsvpReply: h.rsvpReply,
        tier: h.tier,
        rsvpRepliedAt: h.rsvpRepliedAt,
        rsvpSubmittedAt: h.rsvpSubmittedAt,
        stdRepliedAt: h.stdRepliedAt,
        rsvpMessage: h.rsvpMessage,
